...
async function main() {
  ...
  if (doRegistry) {
    ...
    // Initialize with new DID-keyed schema
    console.log(`  Initializing registry (new)...`);
    try {
      ...
      console.log(`  Initialized. Admin: ${MASTER_ACCOUNT}`);
    } catch (e: any) {
      const msg = e.message || '';
      if (msg.includes('already initialized') || msg.includes('Contract already initialized')) {
        console.log(`  Already initialized.`);
      } else {
        console.warn(`  Init warning:`, msg.substring(0, 300));
      }
    }
