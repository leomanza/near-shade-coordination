...
export async function getEncryptedClient() {
  if (!_encryptedClientPromise) {
    _encryptedClientPromise = (async () => {
      ...
      console.log(`[vault] Encrypted client ready (Lit + Storacha, gateway: ${PRIMARY_GATEWAY})`);
      return client;
    })();
  }
...
export async function retrieveAndDecrypt(
  ...
  for (const gateway of allGateways) {
    for (let attempt = 1; attempt <= RETRIES_PER_GATEWAY; attempt++) {
      try {
        console.log(
          `[vault] Retrieving CID ${cidString} via ${gateway}` +
        ...
        console.warn(
          `[vault] Gateway ${gateway} failed (attempt ${attempt}/${RETRIES_PER_GATEWAY}): ${errMsg.slice(0, 120)}`
        );
  ...
  throw new Error(
    `[vault] All gateways failed for CID ${cidString}:\n${summary}`
  );
}
