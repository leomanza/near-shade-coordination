...
import type { ProvisionStatus } from "../hooks/useProvisionJob";
...
const STEPS: Array<{ key: ProvisionStatus; label: string }> = [
  { key: "generating_identity", label: "Generating worker identity" },
  ...
  { key: "awaiting_near_signature", label: "Registering on NEAR" },
];

function getStepState(
  stepKey: ProvisionStatus,
  currentStatus: ProvisionStatus
): "done" | "active" | "pending" {
  ...
  return (
    <div className="rounded border border-[#00ff41]/10 bg-[#0a0f0a]/80 p-6 terminal-card">
      <h3 className="text-sm font-semibold text-zinc-100 mb-1 font-mono">
        Deploying {displayName ? `"${displayName}"` : "your worker"}...
      </h3>
