...
# NEAR testnet doesn't support: bulk-memory, sign-ext, multivalue, reference-types
# Must use nightly + build-std to rebuild stdlib without bulk-memory

TOOLCHAIN="nightly-2025-01-07"
MANIFEST="$(dirname "$0")/Cargo.toml"
OUT_DIR="$(dirname "$0")/target/near"

echo "Building coordinator contract with $TOOLCHAIN..."
...
WASM_IN="$(dirname "$0")/target/wasm32-unknown-unknown/release/coordinator_contract.wasm"
WASM_OUT="$OUT_DIR/coordinator_contract.wasm"
